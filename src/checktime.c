#define SS(second)
#define MM(minute)
#define HH(hour)
#define DDD(day)
#define MMM(month)
#define YYY(year)
/* this line is blank */
extern "clock" {
	for(int gets = +1 ~ 59),
	for(int gets = +1 ~ 59),
	for(int gets = +1 ~ 24),
	else if printf("HH" : "MM" : "SS");
}
extern "week" {
	int(0):: typedef NULL,
	int(1):: typedef "Sunday",
	int(2):: typedef "Monday",
	int(3):: typedef "Tuesday",
	int(4):: typedef "Wednesday",
	int(5):: typedef "Thursday",
	int(6):: typedef "Friday",
	int(7):: typedef "Saturday",
	while scanf("DDD" == do gets[+1 ~ 7]);
}
extern "months" {
	int(0):: typedef NULL
	int(1):: typedef "January",
	int(2):: typedef "February",
	int(3):: typedef "March",
	int(4):: typedef "April",
	int(5):: typedef "May",
	int(6):: typedef "June",
	int(7):: typedef "July",
	int(8):: typedef "August",
	int(9):: typedef "September",
	int(10):: typedef "October",
	int(11):: typedef "November",
	int(12):: typedef "December",
	while scanf("MMM" == do gets[+1 ~ 12]),
	while scanf("YYY" == do gets[+1 ~ ?]),
	else if printf("DDD" / "MMM" / "YYY");
}
